...
For this example the folder and file structures will look similar to this.

![](images/folderstructure.png)

### Documents
After installing the QCoder package organize your data. 
We _suggest_ but do not require using the `create_qcoder_project()` function. 
If you don't use the function, create a folder to store your 
documents.  In our example we'll call this "documents."
Inside that folder put each individual document that will be coded. At this
point we only have tested support for txt files though other types 
will probably work.  If you have documents in other
formats you can use "Save As" to convert to txt. 

### Codes

QCoder has the option to import a list of predefined codes, currently from a CSV
file (if you have this in a spreadsheet you can "Save As" csv).  This file should
haveexactly 3 columns with headings:  
 ...
 Here is the example csv file that comes with QCoder.
...
you to produce a detailed codebook. (Qcoder will include a way to add new codes
to this file for users who wish to do so.)

If you have created a project, story this file in the codes folder.

(There will be support for importing two additional basic data files. 
The first is a list of "units" such as people or organizations along with any
associated unit-level data.  The second is a list connecting units to specific 
documents.  This will allow multiple units (people) to be associated with a
single document.)
 
We have already created sample folders and files listed above.

To get the full paths to the sample data for the codes and the 
folder containing the sample documents type the following.

```
system.file("example_codes/example_codes.csv",  package = "qcoder")

system.file("Example_Data_Markedup",  package = "qcoder")
```
You will also need the full path for the location where you will store the 
imported data, which will be the data_frames subfolder of your project folder
if you created one.  You will need to add a name for the imported document data
if you want it to be different than qcoder_documents. 

You will need the path to your projects data_frames folder.

Once you know this information launch the shiny data importer.

```
shiny::runApp('shiny/data-reader')
```
![](images/data-reader-app.png)

Put the code information into the appropriate spaces and click the button to import the codes.  
Then put the document information and click the corresponding button.  
